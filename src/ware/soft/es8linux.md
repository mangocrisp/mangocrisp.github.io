...
shortTitle: ELK8 Linux 安装
# 当前页面内容描述
description: ELK8 Linux 安装
# 是否在侧边栏或目录中索引当前页面
index: true
# 当前页面是否开启评论功能
comment: true
...
::: info
在windows上安装可以自动生成，在 linux 上不会自动生成证书这些东西，需要自己配置！！！
:::
...
```bash
./bin/elasticsearch-certutil csr -name kibana-server -dns mangocrisp.github.io,localhost
```
...
input {
    ...
    jdbc {
        type => "taybct_api_log"
        # 数据库连接地址
        jdbc_connection_string => "jdbc:mysql://127.0.0.1:3306/taybct?useSSL=false&useUnicode=true&characterEncoding=utf8&useOldAliasMetadataBehavior=true&useTimezone=true&serverTimezone=GMT%2B8&useLegacyDatetimeCode=false&allowMultiQueries=true"
        # 数据库连接账号密码；
        jdbc_user => "by-user"
